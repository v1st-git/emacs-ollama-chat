;;	$Id: ollama-chat-md-mode.el,v 1.10 2025/08/01 07:08:40 vst Exp vst $	
...
;; Prerequisites: the following packages should be installed:
(require 'json)
(require 'cl-lib)
(require 'url)
...
(defgroup ollama-chat-md-mode nil
  "Ollama chat client for Emacs using Markdown mode."
...
(defcustom ollama-chat-md-mode:model-name "gemma3:12b" ;; "phi4:latest" etc...
  "Ollama model."
  :group 'ollama-chat-md-mode
  :type 'string)
...
and prints the answer, received from Ollama API"
  ...
  (ollama-chat-type (list ollama-chat-md-mode:bot-keystring
		     "<!--" "Here you can interact with LLM model by Ollama API."
			  ...
			  "'ollama-chat-md-mode'"
			  (concat "Model name:" ollama-chat-md-mode:model-name)
...
(defun ollama-chat-readin ()
  "Read a sentence from the last human keystring to the end in the current buffer.
   Return it as a string after removing of surrounding blanks."
  (goto-char (point-max))
  ...
  (let ((sentence
	 (string-trim (buffer-substring-no-properties (point) (goto-char (point-max))))
	 ))
    sentence
    ))
  ...
  (let ((tag_name (intern tag_string)))
	...
	(cdr
	 (assoc tag_name
      ...
      (error
       (concat
	(format "\nollama-chat-get-json-tag-from-string error occurs: %s\n"
